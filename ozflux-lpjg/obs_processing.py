...
def get_lai_data(data: pandas.DataFrame, site: str, timestep: int, pcb: Callable[[float], None]) -> list[Observation]:
    ...
    """
    data = modis_get_site_data(data, site)
    data = modis_normalise(data)
    data = modis_qcfilter(data)
    data = modis_guard(data)

    lai_col = modis_get_lai_col()
    lai = [Observation(datetime.datetime.combine(date.date(), datetime.datetime.min.time()), float(row[lai_col])) for (date, row) in data.iterrows()]
    return lai
...
def main(opts: Options):
    ...
    lai_data = modis_read_lai(opts.lai_file)
    greenness_data = modis_read_greenness(opts.greenness_file)
